...
export function reqCurrentUserChats(CurrentUser, setAllUsersChat ,allUsersChats) {
    ...
    if (!Object.keys(allUsersChats).includes(CurrentUser.userName)){
        ...
        // xhr.open("GET",`/getChats?targetId=${CurrentUser.userName}&reqId=${localStorage.getItem("uid")}`);
        xhr.open("GET",`${endURL}/getChats?targetId=${CurrentUser.userName}&reqId=${localStorage.getItem("uid")}`);
